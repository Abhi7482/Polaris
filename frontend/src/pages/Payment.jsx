    ...
    return (
        <div className="h-screen w-screen flex flex-col items-center justify-center bg-polaris-bg relative z-50 overflow-hidden">
            ...
            >
                ...
                <button
                    onClick={handleBack}
                    className="absolute top-10 left-10 flex items-center gap-4 text-polaris-muted hover:text-polaris-primary transition-all duration-300 group"
                >
                    <div className="p-4 rounded-full bg-polaris-bg/50 group-hover:bg-polaris-accent/20 transition-colors">
                        <ArrowLeft className="w-8 h-8 md:w-10 md:h-10" />
                    </div>
                    <span className="font-medium text-xl md:text-2xl tracking-wide uppercase hidden sm:block">Back</span>
                </button>

                <h2 className="font-display text-5xl md:text-7xl font-light mb-8 text-polaris-text type-hero mt-12 md:mt-0">Checkout</h2>
                <p className="text-polaris-muted mb-16 text-2xl md:text-4xl text-balance font-light">Total: ₹{copies * 100}</p>
                ...
                {error && (
                    <motion.div
                        ...
                        className="bg-polaris-accent/20 text-polaris-primary p-6 rounded-2xl mb-10 text-xl font-medium"
                    >
                        {error}
                    </motion.div>
                )}
                ...
                <button
                    ...
                    className="w-full btn-magnetic bg-polaris-primary hover:bg-polaris-text text-polaris-white font-semibold py-8 px-12 rounded-3xl text-3xl md:text-4xl transition-all duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed shadow-2xl flex items-center justify-center gap-4 hover:scale-[1.02] active:scale-[0.98]"
                >
                    {loading ? (
                        <>
                            <svg className="animate-spin h-10 w-10 text-polaris-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                {import.meta.env.DEV && (
                    <button
                        ...
                        className="mt-12 text-sm md:text-base text-polaris-muted hover:text-polaris-primary underline transition-colors duration-200"
                    >
                        [DEV] Skip Payment
                    </button>
                )}
